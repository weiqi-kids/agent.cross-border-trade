...
for category in bilateral_flow product_detail country_profile hs_section; do
...
if [[ -n "${QDRANT_URL:-}" ]]; then
  if qdrant_init_env; then
    ...
    for md_file in "${MD_FILES[@]}"; do
      ...
      source_url="ots://open_trade_stats/${category}/$(basename "$md_file" .md)"
