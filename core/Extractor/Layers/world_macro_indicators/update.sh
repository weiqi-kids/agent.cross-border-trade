...
for category in trade_gdp_ratio trade_openness gdp_growth current_account; do
...
if [[ -n "${QDRANT_URL:-}" ]]; then
  if qdrant_init_env; then
    ...
    for md_file in "${MD_FILES[@]}"; do
      ...
      source_url="worldbank://world_macro_indicators/${category}/$(basename "$md_file" .md)"
