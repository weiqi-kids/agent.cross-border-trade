...
1. 讀取 `docs/Narrator/supply_chain_analysis/` 下前 3 個月報告
2. 僅提取「執行摘要」和「一、關鍵發現」區塊
3. 檔案不存在時跳過，在「已知限制」中說明

**整合方式**：
- 「1.1 數據摘要」表格加入「上月」「3 月平均」欄位
- 「二、集中度風險分析」標註趨勢箭頭（↑↓→）

#### 去年同期（同月）

1. 查找 `{YYYY-1}-{MM}-supply-chain-analysis.md`
2. 用於季節性調整和長期趨勢判斷
...
**查詢設計**：基於本期重點議題生成 5-8 個查詢

| 查詢主題 | 目標 |
|---------|------|
| 供應鏈集中度風險 | 識別類似的歷史風險事件 |
| 稀土/關鍵礦物管制 | 追蹤政策演進脈絡 |
| 半導體供應鏈 | 捕捉產業動態 |
| 美中貿易關係 | 地緣政治背景 |
| 鋰電池材料 | 新能源供應鏈追蹤 |
